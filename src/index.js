...
import { Attr, AttrType, Class, MAGIC_COOKIE, Method, Stun } from './stun.js';
...
if (import.meta.main) {
	...
	packet_loop: for (;;) {
		...
		if (msg.class == Class.Ind && msg.method == Method.Send) {
			...
			if (!(peer?.ip instanceof Ip6) || !data?.byteLength) {
				debugger;
				continue packet_loop;
			}
			...
			hosted: {
				if (data[0] < 20) {
					/* STUN */ const inner = new Stun(data);
					if (inner.byteLength != length) {
						debugger;
						continue packet_loop;
					}
					...
					if (inner.class == Class.Req) {
						...
						if (is_broadcast) {
							...
							if (entry instanceof CryptoKey) {
								...
								inner.msg.length = -Stun.minByteLength + integrity.byteOffset -
									inner.byteOffset + integrity.byteLength;
								if (
									inner.byteOffset + inner.byteLength +
											(24 /* Peer */ + 28 /* SCTP overhead */ +
												60 /* TODO DTLS overhead */) > buffer.byteLength
								) {
									continue packet_loop;
								}
						...
						if (!mapped || !integrity || !fingerprint || unknown.length) {
							debugger;
							continue packet_loop;
						}
						...
						if (!error || !integrity || !fingerprint || unknown.length) {
							debugger;
							continue packet_loop;
						}
