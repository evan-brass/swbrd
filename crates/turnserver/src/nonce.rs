...
fn expected_tag(ts: u32, client: &SocketAddrV6) -> u64 {
	let mut hash = Sha256::new();
	hash.update(ts.to_ne_bytes());
	hash.update(client.ip().octets());
...
pub fn issue_nonce(mapped: &SocketAddrV6, dur: Duration) -> [u8; 24] {
	use std::io::Write;
	let ts = (STARTUP.elapsed() + dur).as_secs() as u32;
	let tag = expected_tag(ts, mapped);

	let mut ret = [0u8; 24];
	write!(&mut ret[..], "{ts:08x}{tag:016x}").unwrap();
	ret
}
pub fn verify_nonce(nonce: &Parsed<&[u8; 24]>, mapped: &SocketAddrV6) -> bool {
