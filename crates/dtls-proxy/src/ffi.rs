...
pub fn read(ssl: &mut Ssl, buf: &mut [u8]) -> SslIo {
    ...
    classify(p, unsafe { SSL_read(p, buf.as_mut_ptr().cast::<c_void>(), len) })
}

pub fn write(ssl: &mut Ssl, buf: &[u8]) -> SslIo {
    let p = ssl.as_ptr();
    let len = buf.len().min(c_int::MAX as usize) as c_int;
    classify(p, unsafe { SSL_write(p, buf.as_ptr().cast::<c_void>(), len) })
}
...
impl BioPair {
    pub fn new() -> Option<Self> {
        ...
        let rc =
            unsafe { BIO_new_bio_dgram_pair(&mut ssl_end, PAIR_BUF, &mut app, PAIR_BUF) };
    ...
    pub fn drain(&self) -> Vec<Vec<u8>> {
        ...
        loop {
            let n = unsafe { BIO_read(self.app, buf.as_mut_ptr().cast::<c_void>(), buf.len() as c_int) };
            if n <= 0 {
                break;
            }
...
mod tests {
    ...
    fn dgram_bio_write_on_connected_socket() {
        ...
        assert_eq!(n, 5, "BIO_write failed: {}", std::io::Error::last_os_error());
        ...
        assert_eq!(n, 5, "BIO_write after read failed: {}", std::io::Error::last_os_error());
